...
def test_flags_needs_lids_when_item_requires_lids(make_catalog_item):
    out = validate_rules(LineItem(sku="DELI-16", quantity=3), make_catalog_item(requires_lids=True))
    assert Flag.NEEDS_LIDS in out.flags


def test_does_not_flag_needs_lids_when_item_does_not_require_lids(make_catalog_item):
    out = validate_rules(LineItem(sku="X", quantity=3), make_catalog_item(requires_lids=False))
    assert Flag.NEEDS_LIDS not in out.flags
...
def test_returns_a_new_item_without_mutating_the_input(make_catalog_item):
    ...
    out = validate_rules(item, make_catalog_item(min_order=2, requires_lids=True))
...
def test_does_not_duplicate_a_flag_that_already_exists(make_catalog_item):
    item = LineItem(sku="DELI-16", quantity=3, flags=[Flag.NEEDS_LIDS])
    out = validate_rules(item, make_catalog_item(requires_lids=True))
    assert out.flags.count(Flag.NEEDS_LIDS) == 1